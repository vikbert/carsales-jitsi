<script>
  ...
  function encrypt(text, pass) {
    return Tea.encrypt(text, pass);
  }

  function decrypt(secret, pass) {
    return Tea.decrypt(secret, pass);
  }

  function handleSubmit(event) {
    event.preventDefault();

    let enc = encrypt(user, password);
    let dec = decrypt('sKPBhmEeRkU=', password);
    console.table({ enc, dec, user });
    try {
      isLoginValid.update(() => dec === user);
    } catch (error) {}
  }
</script>
...
<div class="login-container">
  <form on:submit={handleSubmit}>
    <div class="card is-rounded px-2">
      <div class="card-title text-centered bold">
